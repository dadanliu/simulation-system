...
- [x] 实现 `apps/bff` 中的 `/api/commodity/list`
- [x] 实现 `apps/server` 中的商品列表接口
- [x] 列表页首屏数据可加载
- [x] 列表展示商品基础信息
- [x] 支持分页
- [x] 支持筛选
...
- [x] 实现 `apps/bff` 中的 `/api/commodity/:id`
- [x] 实现 `apps/server` 中的商品详情接口
- [x] 详情页支持动态路由
- [x] 详情页可展示商品基础详情信息
- [x] 无效 `id` 时可返回异常结果
- [x] 页面可处理详情加载失败
...
- [x] 实现 `apps/bff` 中的 `/api/commodity/create`
- [x] 实现 `apps/server` 中的创建商品接口
- [x] 创建页包含基础表单
- [x] 表单支持必填校验
- [x] 表单支持提交状态
- [x] 提交成功后跳转详情页或列表页
...
- [x] `客户端说明.md` 说明如何启动项目
- [x] `客户端说明.md` 说明项目目录结构
- [x] `架构说明.md` 说明页面层 / BFF 层 / backend 层职责
- [ ] 输出页面访问链路图
- [ ] 输出 BFF 请求链路图
- [ ] 输出登录链路图
- [ ] 输出 30 秒讲解稿
