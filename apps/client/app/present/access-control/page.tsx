...
import { getPermissionList, getRoleList, getUserList } from "@/src/features/user/server";
import { AccessControlClient } from "./access-control-client";

export const dynamic = "force-dynamic";

const REQUIRED_PERMISSIONS = ["user:manage", "role:manage", "permission:manage"];

export default async function AccessControlPage() {
  const currentUser = await getCurrentUser("/present/access-control");

  if (!REQUIRED_PERMISSIONS.every((permission) => currentUser.permissions?.includes(permission))) {
