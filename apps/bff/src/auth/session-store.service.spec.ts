...
jest.mock("ioredis", () => {
  ...
  smembers(key: string) {
    return Promise.resolve([...(mockRedisState.sets.get(key) ?? new Set<string>())]);
  }
...
describe("SessionStoreService", () => {
  ...
  it("stores sessions in redis with TTL and reads them after service restart", async () => {
    ...
    expect(mockRedisState.strings.get(`next-bff:session:${sessionId}`)?.expiresAt).toBeGreaterThan(Date.now());
  ...
  it("returns null for expired sessions", async () => {
    ...
    const service = new SessionStoreService(createConfig({ SESSION_TTL_SECONDS: "1" }));
    const sessionId = await service.createSession("u_admin_001");
  ...
  it("supports multiple sessions per user", async () => {
    const service = new SessionStoreService(createConfig());
    const firstSessionId = await service.createSession("u_admin_001", { userAgent: "Chrome" });
    const secondSessionId = await service.createSession("u_admin_001", { userAgent: "Safari" });

    const sessions = await service.listUserSessions("u_admin_001");

    expect(sessions.map((session) => session.sessionId).sort()).toEqual([firstSessionId, secondSessionId].sort());
    expect(sessions.map((session) => session.device.userAgent).sort()).toEqual(["Chrome", "Safari"]);
  });
});
