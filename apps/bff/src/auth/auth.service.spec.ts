...
describe("AuthService", () => {
  ...
  function createService() {
    ...
    const loginRiskService = {
      ...
      isRateLimitError: jest.fn(
        (error: unknown) => error instanceof HttpException && error.getStatus() === HttpStatus.TOO_MANY_REQUESTS
      ),
      recordFailure: jest.fn(),
      reset: jest.fn()
    };
  ...
  it("creates a session, resets failure counters and records a success log on valid login", async () => {
    const { loginAuditLogService, loginRiskService, service, sessionStoreService, userService } = createService();
    ...
    await expect(
      service.login("Admin", "admin123", { ip: "127.0.0.1", traceId: "trace-login", userAgent: "jest" })
    ).resolves.toEqual({
  ...
  it("records a failure log and returns 401 for invalid credentials before lock threshold", async () => {
    const { loginAuditLogService, loginRiskService, service, userService } = createService();

    userService.findUserByCredentials.mockResolvedValue(null);
    loginRiskService.recordFailure.mockRejectedValue(new UnauthorizedException("invalid username or password"));

    await expect(service.login("admin", "bad-password", { ip: "127.0.0.1" })).rejects.toBeInstanceOf(
      UnauthorizedException
    );

    expect(loginAuditLogService.recordFailure).toHaveBeenCalledWith("admin", expect.objectContaining({ ip: "127.0.0.1" }));
  });
  ...
  it("records a blocked log and returns 429 after lock threshold", async () => {
    ...
    await expect(service.login("admin", "bad-password", { ip: "127.0.0.1" })).rejects.toMatchObject({
      status: HttpStatus.TOO_MANY_REQUESTS
    });

    expect(loginAuditLogService.recordBlocked).toHaveBeenCalledWith("admin", expect.objectContaining({ ip: "127.0.0.1" }));
  });
});
